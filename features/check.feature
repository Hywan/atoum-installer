  ...
  Scenario: open_basedir restriction (no php.ini)
    Given I run "php -n -dopen_basedir=/tmp ./installer --check --global"
     Then I should see output matching
        ...
        >>>> ATOUM_PATH=\/an\/allowed\/path \.\/installer or curl https\:\/\/raw\.github\.com\/gist\/3934569\/installer \| ATOUM_PATH=\/an\/allowed\/path php
        >>>> or
        >>>> php \-dopen_basedir= \.\/installer or curl https\:\/\/raw\.github\.com\/gist\/3934569\/installer \| php \-dopen_basedir=
        >>> You don't have required permissions on \/usr\/local\/bin, \/usr\/share
        """